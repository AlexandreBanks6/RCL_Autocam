...
dataDir = "C:\Users\Randy\Documents\Github\RCL_autocam\dataCollection\ParticipantData";
...
for x = 1:length(participantsToInclude)
    ...
    % Loop through each condition and extract the necessary data
        for i = 1:length(conditions)
            ...
            participantData.(condition) = participantData.(condition)(end-2:end, :);
        end
...
data = summaryStatistics(:, [1,3,5]); % n rows for clusters, 3 columns for bars in each cluster
std_dev = summaryStatistics(:,[2,4,6]); % Random standard deviation values
...
xlabel('Participants');
ylabel('Zscore');
title('Zscore for Each Participant and Condition');
